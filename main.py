...
def export_workpaper_pdf(run_id: int, payload: WorkpaperUpdate, db: Session = Depends(get_db)):
    ...
    title_style = ParagraphStyle('TitleStyle', parent=styles['Heading1'], fontName='Helvetica-Bold', fontSize=24, textColor=HexColor('#0F172A'), spaceAfter=4)
    ...
    body_style = ParagraphStyle('BodyStyle', parent=styles['Normal'], fontName='Helvetica', fontSize=10.5, leading=16, textColor=body_textColor, spaceAfter=8)
    body_center_style = ParagraphStyle('BodyCenter', parent=body_style, alignment=TA_CENTER)

    Story = []
    
    # Header Section (Platform Blue SOX ITGC emphasis)
    Story.append(Paragraph("<font color='#2563EB'>SOX ITGC</font> Evidence Evaluation Report", title_style))
    ...
    for i, p in enumerate(paras):
        ...
        # Convert **text** to nice blue headers if whole line, else bold
